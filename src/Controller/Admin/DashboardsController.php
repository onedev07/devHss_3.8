...
class DashboardsController extends AppController
{
    ...
    public function index()
    {
		...
		//Afficher les paiements électroniques qui expirent dans plus ou moins 10 jours		
		$this->loadModel('Paiements');
		
		$query = $this->Paiements->find('all')
			->where(['Paiements.datefin <=' => new Time('-10 days')])
		...
		//Afficher les paiements physiques qui expirent dans plus ou moins 10 jours
		$this->loadModel('Paiementphysiques');
		
		$query = $this->Paiementphysiques->find('all')
			->where(['Paiementphysiques.date_fin <=' => new Time('-10 days')])
    ...
    {
        $paiements = $this->Paiements->get($id, [
            'contain' => ['Souscriptions', 'Clients'],
        ]);

        $this->set('paiement', $paiements);
    }
